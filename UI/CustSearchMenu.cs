...
namespace UI
{
    public class CustSearchMenu : IMenu
    {
        ...
        private Customers filter = new Customers();
        ...
        public void EditCustomer(Customers toBeChanged)
        {
            ...
            Console.WriteLine("Birthday: "+toBeChanged.cBDay);
            Console.WriteLine("Age: "+toBeChanged.ageNullIfZero);
            Console.WriteLine("Consoles: "+Program.StringListOneLine(toBeChanged.cSystems));
            Console.WriteLine("---------------------------");
            ...
            Console.WriteLine("[6] to edit customer age in format MM-dd-yyyy");
            Console.WriteLine("[7] to add or remove a console this customer owns");
            Console.WriteLine("[8] to remove customer data from database (Customer ID will remain)");
            string choice = Console.ReadLine();
            Console.WriteLine("--------------------");
            switch (choice)
            {
                ...
                case "6":
                    ...
                    toBeChanged.cBDay=Console.ReadLine();
                    break;
                case "7":
                    toBeChanged.cSystems.Clear();
                    cSystems = new Systems();         
                    EditSystems(toBeChanged);
                    break; 
                case "8":
                    Console.WriteLine("WARNING! You have chosen to delete this customer from the database");
                    Console.WriteLine("To confirm this decision please enter the following word: Delete");
                    if (Console.ReadLine()=="Delete")
                    {
                        _custBL.DeleteCustomer(toBeChanged);
                        Console.WriteLine("Customer "+toBeChanged.cID+" removed from the database");
                    }   else
                    {
                        Console.WriteLine("Customer "+toBeChanged.cID+" not removed from the database");
                    }
                    break;
                default:
                    Console.WriteLine("That input was not valid, please try again!");
    ...
    private void EditSystems(Customers cEdited)
        {
            bool test = true;
            string input;
            while (test)
            {
                Console.Clear();
                int stepper=0;
                Console.WriteLine("Consoles: "+Program.StringListOneLine(cEdited.cSystems));
                ...
                {
                    cEdited.cSystems.Add(cSystems._availSystems[toIntInput-1]);
    ...
    private void FilterCustomer()
    {
        bool loop = true;
            ...
            Console.WriteLine("Birthday: "+filter.cBDay);
            Console.WriteLine("Age: "+filter.ageNullIfZero);
            Console.WriteLine("Consoles: "+Program.StringListOneLine(filter.cSystems));
            Console.WriteLine("---------------------------");
            ...
            Console.WriteLine("[6] to filter customer age in format MM-dd-yyyy");
            Console.WriteLine("[7] to add or remove a filter for consoles this customer owns");
            string choice = Console.ReadLine();
            Console.WriteLine("--------------------");
            switch (choice)
            {
                ...
                case "6":
                    Console.WriteLine("Birthday in MM-dd-yyyy: ");
                    filter.cBDay=Console.ReadLine();
                    break;
                case "7":
                    filter.cSystems.Clear();
                    cSystems = new Systems();         
                    EditSystems(filter);
                    break; 
                default:
